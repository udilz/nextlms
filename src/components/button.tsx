...
const style = tv({
  base: "block w-full text-sm border rounded-lg font-semibold shadow-sm shadow-slate-200 transition duration-200 disabled:opacity-50",
  variants: {
    variant: {
      primary: "border-indigo-680 bg-indigo-600 text-indigo-50 hover:bg-indigo-500 active:bg-indigo-700",
...
export const Button = (props: Props) => {
  return (
    <button {... props} className={twMerge(style({... props}), props.className)}>{props.children}</button>
  )
}
